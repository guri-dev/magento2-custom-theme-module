...
use Pilot\Smile\Model\ResourceModel\Slider as ResourceSlider;
use Pilot\Smile\Model\ResourceModel\Slider\CollectionFactory as SliderCollectionFactory;
...
class SliderRepository implements SliderRepositoryInterface
{
    ...
    public function __construct(
        ...
        \Pilot\Smile\Api\Data\SliderInterfaceFactory $dataSliderFactory,
    ...
    public function save(Data\SliderInterface $slider)
    {
        if (empty($slider->getStoreId())) {
            $slider->setStoreId($this->storeManager->getStore()->getId());
        }
    ...
    public function getById($slideId)
    {
        ...
        $this->resource->load($slider, $slideId);
        if (!$slider->getId()) {
            throw new NoSuchEntityException(__('The CMS block with the "%1" ID doesn\'t exist.', $sliderId));
        }
        return $slider;
    }
